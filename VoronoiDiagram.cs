...
public class VoronoiDiagram
{
        ...
        while (events.Count > 0) {
            ...
            switch (simpleEvent.eventType) {
                case Event.EventType.Dot:
                    ...
                    if (SameValue( simpleEvent.directrix, startDirectrix )) {
                        ...
                        if (arcs.Count == 0) {
                            ...
                            if (arcNode == null) {
                                borders.Add( new Border() { siteLeft = arcs.Last.Value, siteRight = simpleEvent.dot } );
                                arcs.AddLast( simpleEvent.dot );
                            } else {
                                borders.Add( new Border() { siteLeft = simpleEvent.dot, siteRight = arcNode.Value } );
                        ...
                        for (int cornerId = 0; cornerId < corners.Count; cornerId++) {
                            ...
                            if (SameValue( corner.dot.x, simpleEvent.dot.x )) {
                                ...
                                Border border = SearchBorder( borders, corner.leftArcNode.Value, corner.rightArcNode.Value );
                                ...
                                borders.Add( new Border() { siteLeft = corner.leftArcNode.Value, siteRight = simpleEvent.dot } );
                ...
                case Event.EventType.Circle:
                    ...
                    Border leftBorder = SearchBorder( borders, leftArcNode.Value, circle.arc.Value );
                    Border rightBorder = SearchBorder( borders, rightArcNode.Value, circle.arc.Value );
                    ...
                    borders.Add( new Border() { siteLeft = leftArcNode.Value, siteRight = rightArcNode.Value, begin = simpleEvent.circle.center } );
    ...
    private class DotLineSurroundings
    {
        public Dot dot;
        public float axisVal;
        //public Border borderIn, borderOut;
    ...
    {
        if (Math.Abs( focus.y - directrix ) < eps) {
    ...
    private static Dot Get3DotsCircleCenter(Dot v1, Dot v2, Dot v3)
    {
        ...
        if (Math.Abs( diff1 ) < eps) {
            if (Math.Abs( diff2 ) < eps || Math.Abs( diff3 ) < eps) {
